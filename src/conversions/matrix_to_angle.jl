...
# ==============================================================================
#
#   Functions related to the conversion from Matrix to Euler angles.
#
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #

export matrix_to_angle

"""
    matrix_to_angle(dcm::Matrix, rot_seq::Symbol=:ZYX)
...
```jldoctest
julia> D = [1. 0. 0.; 0. 0. -1; 0. -1 0.];

julia> matrix_to_angle(D,:XYZ)
EulerAngles{Float64}:
  R(X) :  1.5708 rad  ( 90.0°)
  R(Y) :  0.0    rad  ( 0.0°)
  R(Z) :  0.0    rad  ( 0.0°)
...
```
"""
function matrix_to_angle(dcm::Matrix{T}, rot_seq::Symbol=:ZYX) where T<:Number
    if rot_seq == :ZYX
